...
function buildParametrableQuery(schema, params) {
  ...
  const whereFields = schema.fields.map(
    (f) => `${f.name}: { _eq: $cond_${f.name} }`
  )
  // console.log('whereFields: ' + whereFields)
  const selectFields = schema.fields.map((f) => `${f.name}`)
  const distinctField = params.distinct_on
    ? `distinct_on: [${params.distinct_on.map((el) => `${el}`).join(', ')}]`
    : undefined
  ...
  const extraFields = [distinctField]
